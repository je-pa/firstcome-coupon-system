...
---
# 요구사항 추가
- 현재: 한 명이 여러 개의 쿠폰 중복 발급이 가능하다.

대부분의 선착순 이벤트는 1인당 개수가 1개로 제한되는 경우가 많으니 

**1인단 발급 가능 쿠폰 개수를 1개로 제한하자.**

## 1인당 발급 가능 쿠폰 개수 1개 제한하는 방법
1. UNIQUE KEY 사용
   - userId 와 coupon type 컬럼에 UNIQUE KEY 제약사항 추가하기
   - 단점: 한 유저가 같은 타입의 쿠폰을 여러개 가질 수 없게됨.
2. 범위로 락을 잡고 처음 쿠폰 발급 여부를 가져와서 판단하는 방식
    ```java
    public void apply(Long userId) {
      // lock start
      // if(쿠폰 발급여부 확인) return // 발급됐다면 return
    ...
    ```
   해당 방법은 실제 쿠폰 생성은 컨슈머에서 생성하므로 동시성 이슈가 발생할 수 있다.(=한 명이 두개 발급될 수 있다.)
    
   > 유저 1 확인 -> 쿠폰 발급요청 -> 유저 1 또 확인 -> 쿠폰 발급요청 -> 쿠폰 발급 완료 2번됨  
   - api에서 직접 쿠폰을 발급해도 락범위가 넓어진다는 단점이 있음.
3. Set 자료구조 이용
   > Set: 값을 유니크하게 저장할 수 있는 자료구조

    Set을 활용하여 **값을 두 번 저장해도 한 개만 남는 특성**을 이용해 존재 여부를 빠르게 확인한다.

# Set 자료구조 사용하기
## Redis Set 실습
### redis-cli 접속
```
docker exec -it {docker ps 해서 얻은 redis container ID} redis-cli
```
### set 추가
```
sadd {key} {value}
```
이렇게 하면 추가된 value의 개수를 리턴한다.
- 예시
    ```
    ...
    (integer) 0
    ```
    위에서 보듯, 두번째 실행했을 때 test라는 key에 1 value가 이미 존재하므로 0을 리턴한다.

## Set 활용하기
### set 관리하는 repository 생성
[AppliedUserRepository.java](api/src/main/java/com/example/api/repository/AppliedUserRepository.java)
